...
def patch_detail(request, project_id, msgid):
        ...
        if is_maintainer and form_name == DeleteNoteForm.name:
            ...
            if is_maintainer and action == 'add-note':
                ...
                if request.user.is_authenticated:
                    ...
                    patch.has_planned_review = (
                        patch.planning_to_review.exists()
                    )
                    patch.save()
                    ...
                    messages.error(
                        request,
                        'You must be logged in to declare review interest.',
                    )
                ...
                if request.user.is_authenticated:
                    ...
                    messages.success(
                        request,
                        'You have removed your interest in reviewing this patch.',
                    )
                else:
                    messages.error(
                        request,
                        'You must be logged in to remove review interest.',
                    )
    ...
    for intention in intentions:
        intention.review_expiry_date = (
            intention.last_time_marked_for_review
            + patch.state.review_intention_expiration_time
        )
        if intention.user == request.user:
            context['planning_to_review'] = True
            context['review_expiry_date'] = (
                intention.last_time_marked_for_review
                + patch.state.review_intention_expiration_time
            )
    ...
    if is_maintainer:
        notes = patch.notes.all()
    else:
        notes = patch.notes.filter(maintainer_only=False)
